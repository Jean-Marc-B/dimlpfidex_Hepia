...
public:
  ...
  /**
   * @brief Writes rules for a given sample into a JSON file.
   */
  static void toJsonGloFile(const std::string &filename, const std::vector<std::vector<Rule>> &rulesPerSample);
};
...
inline std::ostream &operator<<(std::ostream &stream, const Rule &rule) {
  ...
  stream << "   #Antecedents:     " << std::to_string(rule.getAntecedents().size()) << std::endl
         << "   #Covered samples: " << std::to_string(rule.getCoveringSize()) << std::endl
         << "   Fidelity:         " << std::to_string(rule.getFidelity()) << std::endl
