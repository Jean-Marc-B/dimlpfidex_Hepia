...
inline std::ostream &operator<<(std::ostream &stream, const Parameters &p) {
  ...
  for (auto const &x : p.getAllStrings()) {
    ...
    stream << Parameters::getParameterName(x.first) << ":" << std::setw(padValue) << x.second << std::endl;
  }

  for (auto const &x : p.getAllInts()) {
    padValue = pad - static_cast<int>(Parameters::getParameterName(x.first).size());
    stream << Parameters::getParameterName(x.first) << ":" << std::setw(padValue) << std::to_string(x.second) << std::endl;
  }
  ...
  for (auto const &x : p.getAllIntVectors()) {
    padValue = pad - static_cast<int>(Parameters::getParameterName(x.first).size());
    stream << Parameters::getParameterName(x.first) << ":" << std::setw(padValue) << p.vectorToString(x.second) << std::endl;
  }
