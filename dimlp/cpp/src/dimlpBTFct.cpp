...

{
  ...
  printOptionDescription("--train_pred_outfile <str>", "Path to the file where the train predictions will be stored (default: dimlpBTTrain.out)");
  ...
  printOptionDescription("--test_pred_outfile <str>", "Path to the file where the test predictions will be stored (default: dimlpBTTest.out)");
  printOptionDescription("--weights_outfile <str>", "Path to the file where the output trained weights of the model will be stored (default: dimlpBT.wts)");
  printOptionDescription("--attributes_file <str>", "Path to the file containing the labels of attributes and classes");
  printOptionDescription("--stats_file <str>", "Path to the file where the train, test and validation accuracy, as well as the train and test global accuracy will be stored (default: statsDimlpBT.txt)");
  ...
  std::cout << "Execution example :" << std::endl
            << std::endl;
  std::cout << "dimlp.dimlpBT(\"--train_data_file train_data.txt --train_class_file train_class.txt --test_data_file test_data.txt --test_class_file test_class.txt --nb_attributes 16 --hidden_layers 5 --nb_classes 2 --nb_dimlp_nets 2 --weights_outfile weights.wts --with_rule_extraction true --global_rules_outfile globalRules.rls --train_pred_outfile predTrain.out --test_pred_outfile predTest.out --root_folder dimlp/datafiles\")" << std::endl
            << std::endl;
  std::cout << "---------------------------------------------------------------------" << std::endl
            << std::endl;
}
 ...
 * @par
 * <tt>dimlp.dimlpBT('-\-train_data_file train_data.txt -\-train_class_file train_class.txt -\-test_data_file test_data.txt -\-test_class_file test_class.txt -\-nb_attributes 16 -\-hidden_layers 5 -\-nb_classes 2 -\-nb_dimlp_nets 2 -\-weights_outfile weights.wts -\-with_rule_extraction true -\-global_rules_outfile globalRules.rls -\-train_pred_outfile predTrain.out -\-test_pred_outfile predTest.out -\-root_folder dimlp/datafiles')</tt>
 *
