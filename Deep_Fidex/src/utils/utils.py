...
def trainCNN(height, width, nbChannels, nb_classes, model, nbIt, batch_size, model_file, model_checkpoint_weights, X_train, Y_train, X_test, Y_test, train_pred_file, test_pred_file, model_stats, with_leaky_relu=False, remove_first_conv=False):
    ...
    - model: Can be resnet, VGG, VGG_metadatas, VGG_and_big, VGG_and_two_bigs, VGG_and_VGG, VGG_and_nClass_VGGs, big, small, MLP or MLP_Patch indicating the model architecture to use (small is a smaller custom model).
    ...
    if model not in ["resnet", "VGG", "VGG_metadatas", "VGG_and_big", "VGG_and_two_bigs", "VGG_and_VGG", "VGG_and_nClass_VGGs", "small", "big", "MLP", "MLP_Patch"]:
        raise ValueError("The model needs to be one of resnet, VGG, VGG_metadatas, VGG_and_big, VGG_and_two_bigs, VGG_and_VGG, VGG_and_nClass_VGGs, small, big, MLP or MLP_Patch")
    ...
    if model == "VGG_and_two_bigs":
        if len(X_train) != 3 or len(X_test) != 3 or len(height) != 3 or len(width) != 3 or len(nbChannels) != 3:
            raise ValueError("Wrong shape of data when training VGG and big.")
    if model in ["VGG_and_big", "VGG_and_two_bigs"]:
    ...
    if model == "VGG_and_nClass_VGGs":
        ...
        elif model == "VGG_and_two_bigs":
            X_train, meta1_train, meta2_train = X_train
            X_test, meta1_test, meta2_test = X_test
            meta1_train = np.array(meta1_train)
            meta1_test = np.array(meta1_test)
            meta2_train = np.array(meta2_train)
            meta2_test = np.array(meta2_test)
            X_test = np.array(X_test)
        ...
        if model in ["MLP_Patch", "VGG_metadatas", "VGG_and_VGG", "VGG_and_big"]:
            ...
            meta1_train, meta1_val = meta1_train[:split_index], meta1_train[split_index:]
            meta2_train, meta2_val = meta2_train[:split_index], meta2_train[split_index:]
        ...
        if model in ["VGG_metadatas", "VGG_and_VGG", "VGG_and_big"]:
            ...
            print(f"Training set meta1: {meta1_train.shape}")
            print(f"Validation set meta1: {meta1_val.shape}")
            print(f"Test set meta1: {meta1_test.shape}")
            print(f"Training set meta2: {meta2_train.shape}")
            print(f"Validation set meta2: {meta2_val.shape}")
            print(f"Test set meta2: {meta2_test.shape}")
        if model in ["VGG_and_big", "VGG_and_VGG", "VGG_and_two_bigs"]:
            nbChannel_img = nbChannels[0]
        else:
            nbChannel_img = nbChannels
        if (nbChannel_img == 1 and model in ["resnet", "VGG", "VGG_and_big", "VGG_and_two_bigs", "VGG_and_VGG", "VGG_metadatas"]):
            # B&W to RGB
            ...
            if model in ["VGG_and_big", "VGG_and_VGG"]:
                ...
                nbChannels = (3, nbChannels[1], nbChannels[2])
            else:
                nbChannels=3
        ...
        if model in ["MLP_Patch", "VGG_metadatas", "VGG_and_big", "VGG_and_VGG"]:
            ...
            x_train = [x_train, meta1_train, meta2_train]
            x_val = [x_val,meta1_val, meta2_val]
            X_test = [X_test,meta1_test, meta2_test]
    ...
    #     model.summary()
    ...
    elif model == "VGG_and_two_bigs": # A VGG for images and a big for probabilities and another for HOG, reuniting at the end
        if with_leaky_relu:
            raise ValueError("VGG with leakyRelu is not yet implemented.")
        ...
        hog_input = Input(shape=(height[2], width[2], nbChannels[2]))
        z = Resizing(2*height[2], 2*width[2])(hog_input)

        # Première couche de convolution
        z = Conv2D(64, (3, 3), activation=None, padding='same', strides=2, kernel_regularizer=l2(0.0005))(z)
        z = BatchNormalization()(z)
        z = tf.keras.layers.LeakyReLU(alpha=0.1)(z)
        z = MaxPooling2D(pool_size=(2, 2), name='first_conv_end_2')(z)

        # Deuxième couche de convolution
        z = Conv2D(128, (3, 3), activation=None, padding='same', kernel_regularizer=l2(0.0005))(z)
        z = BatchNormalization()(z)
        z = tf.keras.layers.LeakyReLU(alpha=0.1)(z)
        z = MaxPooling2D(pool_size=(2, 2))(z)

        # Troisième couche de convolution
        z = Conv2D(256, (3, 3), activation=None, padding='same', kernel_regularizer=l2(0.0005))(z)
        z = BatchNormalization()(z)
        z = tf.keras.layers.LeakyReLU(alpha=0.1)(z)
        z = MaxPooling2D(pool_size=(2, 2))(z)
        ...
        # FINAL MODEL
        model = Model(inputs=[image_input, probability_input, hog_input], outputs=output)
