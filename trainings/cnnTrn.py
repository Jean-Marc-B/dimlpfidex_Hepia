...
def get_and_check_parameters(init_args):
    ...
    parser.add_argument("--original_input_size", type=lambda x: pair_type(x, dict(func=int_type, min=1)), metavar="<pair<int [1, inf[>>", help="Original input size", required=True)
    ...
    parser.add_argument("--valid_ratio", type=lambda x: float_type(x, min=0, min_inclusive=False, max=1, max_inclusive=False), metavar="<float ]0,1[>", help="Percentage of train data taken for validation (default: 0.1)")
    ...
    parser.add_argument("--valid_class_file", type=lambda x: sanitizepath(args.root_folder, x), help="Path to the file containing the validation true classes of the dataset, mandatory if classes are not specified in valid_data_file. BE CAREFUL if validation files are given, and you want to use Fidex algorithms later, you will have to use both train and validation datas given here in the train datas and classes of Fidex", metavar="<str>")
